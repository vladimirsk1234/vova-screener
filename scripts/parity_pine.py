...
def _compare(ticker: str, direction: str) -> list[str]:
    df = _load_df(ticker)
    if df is None or len(df) < 50:
        return [f"{ticker}/{direction}: skip (no data)"]
    ...
    direction_sell = direction == "sell"

    py = _pine_result_dict(
        _run_sequence_vova_pine_python(c, h, l, atr, 1.5, True, 100.0, direction_sell)
    )
    if not _NUMBA_AVAILABLE or _run_sequence_vova_pine_numba is None:
        return [f"{ticker}/{direction}: numba not available, skip"]

    nb = _pine_result_dict(
        _run_sequence_vova_pine_numba(c, h, l, atr, 1.5, True, 100.0, direction_sell)
    )
    pub = run_sequence_vova_pine(df, min_rr=1.5, direction=direction)

    errs = []
    tag = f"{ticker}/{direction}"
...
def main() -> int:
    ...
    for direction in DIRECTIONS:
        for t in TICKERS:
            all_errs.extend(_compare(t, direction))
    if all_errs:
        for e in all_errs:
            print(f"FAIL: {e}")
        return 1
    print(f"OK: parity passed for {len(TICKERS)} tickers x {len(DIRECTIONS)} directions")
    return 0
