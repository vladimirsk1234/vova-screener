...
- **Рост:** тот же **5y EPS CAGR**, что в `packages/engine/src/fundamentalsValuation.ts` (годы с EPS ≤ 0 не входят). Порог: 15%.
- **Недооцененные (CSV, файл вселенной не сужаем):**
  - `growthRate < 15%` → `peTTM < 15` (и PE > 0)
  - `growthRate ≥ 15%` → Lynch **PEG = peTTM / growthRate < 1**
...
- CSV → `reports/undervalued-pe-peg.csv`: yahoo, tv, name, exchange, epsTtm, peTtm, growth5y, rule (`pe15` / `lynch_peg`), pegLynch, pass
