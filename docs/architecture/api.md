...
| Method | Path | Notes |
|--------|------|-------|
| GET | `/history?tf=Weekly\|Monthly\|All&groupBy=Day\|Weekly\|Monthly&range=all\|ytd\|1m\|3m\|6m\|1y\|max&sort&dir` | Win rate, net P&L, avg R, avg RR at entry, avg hold, equity curve and exit-reason histogram over realized trades; aggregated in Mongo. `range` filters by `exitDate` (`max` ≡ `all`). `sort` = `period`, `pnl`, `winRate`, `trades`, `rr` (avg RR at entry). `groupBy=Day` (legacy alias `Daily`) buckets by calendar exit date — not the retired Daily timeframe. Also carries `timeframes`: Weekly and Monthly with their own trades, win rate, net P&L, avg R and equity curve under the same `range`. Totals include the capital pool (`peakCapitalUsd`, `avgCapitalUsd`, `openCapitalUsd`, `roiOnPeakPct`, `roiOnAvgPct`) and an S&P compare over the same History range (`benchmarkSymbol` is `SPY` or `^GSPC`, Yahoo adjusted close / FMP price fallback; `alphaVsBenchmarkPct` = ROI on peak − S&P, `alphaOnAvgPct` = ROI on avg − S&P). A failed benchmark fetch leaves those fields null. Settings Min RR and `fundamentalsFilter` apply here the same way they do on Results |
...
| GET | `/instruments/fundamentals-screener?stars=&sort=&dir=&limit=&offset` | Value tab. Mongo only. `stars` = `undervalued` (default, ≥1/4) / `0` / `1` / `2` / `3` / `4` / `all`. Response includes `counts`, `coverage` (`universe` / `stored` / `reliable` / `complete`), `lastFullAt`, `lastRun` |
