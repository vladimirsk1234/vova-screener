...
test.describe('chart parity UI', () => {
  ...
  test('TA / Fundamentals toggle keeps the selected timeframe', async ({ page }) => {
    ...
    await expect(page.getByRole('button', { name: 'Op. EPS', exact: true })).toHaveClass(/active/);
    await expect(page.getByRole('button', { name: 'FCF/sh', exact: true })).toBeVisible();
    await expect(page.getByRole('button', { name: 'EPS', exact: true })).toHaveCount(0);
    await expect(page.getByRole('button', { name: 'Sales/sh', exact: true })).toHaveCount(0);
    await expect(page.getByRole('button', { name: 'Owner earn.', exact: true })).toHaveCount(0);
